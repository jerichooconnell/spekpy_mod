...
##################################
import os
from numpy import sin, cos, arctan, arccos, deg2rad, degrees, interp, searchsorted
from numpy import exp, sqrt, log, sum, abs, floor, round, divide, trapz
from numpy import pi, nan, clip
...
def custom_interp1d(x, y):
    ...
    def interpolator(x_new):
        ...
        indices = searchsorted(x, x_new)

        # Ensure indices stay within bounds
        indices = clip(indices, 1, len(x) - 1)
...
class SpekAniso:
    ...
    def __get_range_fn(self):
        ...
        range_fn = custom_interp1d(E0_data, rng_data)#, kind='quadratic', 
                                        # bounds_error = False)
        return range_fn
    ...
    def __get_logmac_fn(self, Rayleigh=True):
        ...
        # ... K/L edges
        logmac_fn = custom_interp1d(logk_data, logmac_data)#, kind = 'linear', 
    ...
    def __Brems(self):
        ...
        for i in range(self.k.size):
            ...
            NE_tip = custom_interp1d( E[ ind_Erng[:,i] ], NE[ ind_Erng[:,i] ].T)
                        # , kind='linear', bounds_error=False, fill_value=0.0)
            # Generate interpolation fn for angular distribution at the 
            # ... tip (the tip region is where k ~ E)
            # The function returns a vector of values for x, for each E value
            fomega_x_tip = custom_interp1d( E[ ind_Erng[:,i] ], 
